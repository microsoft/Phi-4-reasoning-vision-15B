...
# Register "bunny-phi3" model_type to use LlamaConfig
# This allows vLLM to load Bunny checkpoints without patching config.json
...
# Optional: import Bunny's SigLIP2 NAFLEX image processor for parity with training
try:
    from .siglip2_encoder import (
        Siglip2ImageProcessorNoUpscale as _BunnySiglip2ImageProcessor,
    )  # type: ignore
except Exception:
    raise RuntimeError("Bunny SigLIP2 NAFLEX processor import failed.")
...
class LlamaSiglipProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs) -> ProcessorMixin:
        ...
        # Prefer Bunny SigLIP2 NAFLEX processor when NAFLEX detected from config
        naflex_enabled = self.is_naflex_enabled()
        try:
            if naflex_enabled:
                if _BunnySiglip2ImageProcessor is not None:
                    ...
                    image_processor = _BunnySiglip2ImageProcessor(
                        max_num_patches=max_patches,
                        min_num_patches=min_patches,
                    )
                else:
                    root_log.warning("[LlamaSiglipProcessingInfo] NAFLEX enabled but Bunny SigLIP2 processor unavailable; falling back to HF processors.")
        except Exception as e:
            root_log.warning("[LlamaSiglipProcessingInfo] Failed to create Bunny SigLIP2 processor: %s", e)
        # Try fast processor
        if image_processor is None:
...
class LlamaSiglipForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def embed_multimodal(self, **kwargs: object) -> MultiModalEmbeddings:
        ...
        if isinstance(image_input, Mapping) and image_input.get("type") == "naflex":
            ...
            with torch.no_grad():
                # Use select_layers=[-2] to match Bunny training behavior (second-to-last layer)
    ...
    # The bridge class now handles all Phi4/Bunny -> Llama weight transformations
    ...
    def transform_siglip2_weight_names(
        ...
        Bunny uses hidden_states[-2] from the encoder, NOT the pooling head output.
        ...
        """
        # Skip pooling head weights - Bunny doesn't use them (uses hidden_states[-2])
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, tensor in weights:
            weight_count += 1
            if "vision" in name:
                # Strip Bunny prefix: model.vision_tower.vision_tower.X -> X
