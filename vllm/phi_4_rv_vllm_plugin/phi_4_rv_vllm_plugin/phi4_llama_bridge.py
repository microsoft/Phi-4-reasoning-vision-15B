...
"""Phi4/Bunny to Llama bridge model for vLLM.

This module provides a bridge class that allows loading Phi4/Bunny format
...
The Bunny checkpoint uses fused weights (qkv_proj, gate_up_proj) while
...
def _patch_phi4_config_for_llama(hf_config) -> None:
    """Mutate a Phi4/Bunny-style config to be Llama-compatible.
    
    The Bunny config (model_type: bunny-phi3) needs these patches to work
    ...
    """
    # head_dim is calculated, not stored in Bunny config
    # Llama config has head_dim: 128, Bunny has hidden_size: 5120, num_attention_heads: 40
    # 5120 / 40 = 128
...
class Phi4VisionBridgeLlama(LlamaForCausalLM):
    """LlamaForCausalLM that loads Phi4/Bunny format weights directly.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Transform Phi4/Bunny format weights to Llama format, then load.
        
        Filters out vision/projector weights (handled by parent LlamaSiglip).
        
        Args:
            weights: Iterator of (name, tensor) pairs in Phi4/Bunny format
        ...
        def transform_iter():
            ...
            for name, tensor in weights:
                ...
                # Bunny checkpoint: model.layers.X... -> vLLM Llama expects: model.layers.X...
    ...
    def _transform_phi4_weight(
        ...
        """Transform a single Phi4/Bunny weight to Llama format.
        
        Args:
            name: Weight name in Phi4/Bunny format
